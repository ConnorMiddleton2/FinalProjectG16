              ...
              {values.attachments.map((file) => (
                ...
                >
                  <span className="min-w-0 truncate text-[var(--harbor-ink)]">
                    {file.name}{" "}
                    <span className="text-[var(--harbor-ink)]/50">
...
function Field({
  ...
  return (
    <label className="form-control w-full" htmlFor={htmlFor}>
      ...
      {children}
      {hint && !error ? (
        <span className="mt-1 text-xs text-[var(--harbor-ink)]/50">{hint}</span>
      ) : null}
      {error ? (
        <span className="mt-1 text-sm text-error" role="alert">
          {error}
        </span>
      ) : null}
    </label>
  ...
  return (
    <div className="space-y-5 rounded-2xl border border-[var(--harbor-deep)]/10 bg-white/85 p-5 shadow-sm sm:p-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button type="button" className="btn btn-outline btn-sm" onClick={onAnother}>
...
function ConfirmTile({
  ...
  return (
    <div className="rounded-xl bg-[var(--harbor-sand)]/40 px-3 py-3">
      <p className="text-xs uppercase tracking-wide text-[var(--harbor-ink)]/50">
        {label}
      </p>
      <p
