...
export function PropertyDetailView({ contract, onBack }: Props) {
  ...
  const rosterByUnitKey = useMemo(
    () =>
      new Map(rosterTenants.map((rt) => [normalizeUnitKey(rt.unit), rt])),
    [rosterTenants]
  );
  const masterTenantById = useMemo(
    () => new Map(masterTenants.map((t) => [t.id, t])),
    [masterTenants]
  );
            ...
            <table className="table table-sm">
              ...
              <tbody>
                {occupiedRows.map((row) => {
                  const masterTenant = masterTenantById.get(row.tenantId);
                  const rosterMatch = rosterByUnitKey.get(row.unitKey);
                  return (
                    <tr key={row.unitKey}>
                      <td className="font-medium">
                        <p>{row.unitLabel}</p>
                        {rosterMatch?.floorPlan ? (
                      ...
                      <td>
                        {masterTenant
                          ? `${masterTenant.sqft.toLocaleString()} sf`
                          : "—"}
                      </td>
                      <td>
                        <p>
                          {masterTenant
                            ? `$${masterTenant.monthlyRent.toLocaleString()}`
                            : "—"}
                        </p>
                        {rosterMatch?.fairMarketRent ? (
                          <p className="text-xs opacity-55">
                            FMR $
                            {Number(rosterMatch.fairMarketRent).toLocaleString()}
                            {rosterMatch.askingRent
                              ? ` · ask $${Number(rosterMatch.askingRent).toLocaleString()}`
                              : ""}
                          </p>
                        ) : null}
                      </td>
