...
/** Cap typed custom amount length before parse (avoids absurd strings). */
export const CUSTOM_AMOUNT_INPUT_MAX = 12;

export function parseAmountInput(raw: string): number | null {
  const trimmed = raw.trim().replace(/[$,\s]/g, "");
  if (!trimmed) return null;
  if (trimmed.length > CUSTOM_AMOUNT_INPUT_MAX) return null;
